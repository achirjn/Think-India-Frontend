...
export default function UserUpcomingEvents() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError('')
      try {
        // Authenticated endpoint confirmed by backend: /user/upcommingEvents
        const url = 'http://localhost:8082/user/upcommingEvents'
        const res = await authFetch(url, { headers: { 'Accept': 'application/json' } })
        if (!res.ok) throw new Error(`HTTP ${res.status}`)
        const data = await res.json()
        const list = Array.isArray(data) ? data : []

        // For each event, load ALL images from imageIdList for a removable gallery
        const withImages = await Promise.all(list.map(async (ev, i) => {
          ...
          if (Array.isArray(imageIdList)) {
            for (const id of imageIdList) {
              try {
                let imgRes = await authFetch(`http://localhost:8082/image/${encodeURIComponent(id)}`, { headers: { 'Accept': 'application/json, text/plain, */*' } })
                if (!imgRes.ok) imgRes = await authFetch(`http://localhost:8082/image/${encodeURIComponent(id)}`, { mode: 'cors' })
  ...
  return (
    <section className="container-responsive py-responsive">
      <header className="mb-6">
        <h1 className="text-3xl sm:text-4xl font-extrabold text-[color:var(--color-ashoka-blue)]">Upcoming Events</h1>
      </header>
      ...
      {loading ? (
        ...
        <p className="text-[color:var(--color-ashoka-blue)]/80">No upcoming events.</p>
      ) : (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {events.map((ev, idx) => (
            <motion.article 
